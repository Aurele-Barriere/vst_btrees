...
Definition _b : ident := 53%positive.
Definition _main : ident := 55%positive.
Definition _sum : ident := 54%positive.
...
Definition f_sum := {|
  ...
  fn_temps := nil;
  fn_body :=
(Sreturn (Some (Ebinop Oadd (Ecast (Etempvar _a tint) tint)
...
(Ssequence
  (Ssequence
    (Scall (Some _t'1)
      (Evar _sum (Tfunction (Tcons tint (Tcons tint Tnil)) tint cc_default))
      ((Econst_int (Int.repr 2) tint) :: (Econst_int (Int.repr 3) tint) ::
       nil))
...
(_main :: _sum :: ___builtin_debug :: ___builtin_nop ::
