...
Indeed, we don't need to prove anything about the filler \todo{?} values in a C array that are outside of the Coq list that represents it.
...
Similarly, when defining \lstinline{tree_rep t p}, for each child \texttt{c} of \texttt{t}, we claim that there exists some value \texttt{pc} such that the C pointer to \texttt{c} in \texttt{t} contains \texttt{pc} and in separating conjunction, we have \lstinline{tree_rep c pc}.\todo{should I write a complete example?}
